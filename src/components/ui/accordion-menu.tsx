...
function AccordionMenu({
  ...
  React.useEffect(() => {
    if (props.type === 'single' && selectedValue) {
      setNestedStates((prev) => {
        if (prev.root === selectedValue) return prev;
        return { ...prev, root: selectedValue };
      });
    }
  }, [props.type, selectedValue]);
  const multipleValue = (
...
const itemVariants = cva(
  'relative cursor-pointer select-none flex w-full text-start items-center rounded-lg gap-2 px-2 py-1.5 outline-hidden transition-colors disabled:opacity-50 disabled:bg-transparent focus-visible:text-accent-foreground [&_svg]:pointer-events-none [&_svg]:opacity-60 [&_svg:not([class*=size-])]:size-4 [&_svg]:shrink-0 [&_a]:flex [&>a]:w-full [&>a]:items-center [&>a]:gap-2 h-9 border border-transparent text-[color:var(--sidebar-foreground)] hover:bg-[color:var(--sidebar-hover-bg)] hover:border-[color:var(--sidebar-hover-border)] hover:text-[color:var(--sidebar-hover-text)] data-[selected=true]:bg-[color:var(--color-primary)/12%] data-[selected=true]:border-[color:var(--color-primary)] data-[selected=true]:text-[color:var(--color-primary)] data-[selected=true]:shadow-[0_0_0_1px_color:var(--color-primary)] data-[selected=true]:font-semibold focus-visible:bg-[color:var(--sidebar-accent-soft)] focus-visible:border-[color:var(--sidebar-accent-strong)] focus-visible:shadow-[0_0_0_1px_var(--sidebar-ring)] text-[13px]',
  {
    variants: {
      variant: {
        default: '',
